...
import sys
sys.modules['tendrl.ceph_integration.config'] = MagicMock()
from tendrl.ceph_integration.manager import rpc
del sys.modules['tendrl.ceph_integration.config']
import uuid
...
class TestEtcdThread(object):
    ...
    def test_etcdthread(self, monkeypatch):
        ...
        self.etcdthread._complete = self
        sys.modules[
            'tendrl.ceph_integration.flows.create_pool'] = MagicMock()
        self.etcdthread._run()
        del sys.modules[
            'tendrl.ceph_integration.flows.create_pool']
...
class Mock_Job(object):
    def __init__(self):
        ...
        "Package.name": "tendrl-ceph-integration", \
        "Tendrl_context.sds_name": "ceph", "Node[]":[], \
        "etcd_client": ""}, "parent":"",\
